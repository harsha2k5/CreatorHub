<?php
/**
 * CreatorHub - AI Analysis Service (Gemini 2.0 Flash REST API + Deterministic Engine)
 */
...
class AIAnalysisService {
    ...
    private static function computeDeterministicModel(array $creator, int $followers, float $engagementRate): array {
        $engagementScore = min(98, max(55, (int)round($engagementRate * 20)));
        $audienceScore = $followers > 50000 ? 92 : ($followers > 10000 ? 84 : 72);
        $contentScore = 88;
        $consistencyScore = 85;
        $brandSuitability = 90;
        $overallScore = (int)round(($engagementScore + $audienceScore + $contentScore + $consistencyScore + $brandSuitability) / 5);
