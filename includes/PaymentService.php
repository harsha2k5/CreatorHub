<?php
/**
 * CreatorHub - Razorpay Payment & Escrow Service
 */
...
class PaymentService {
    ...
    public static function createEscrowOrder(string $collaborationId, string $brandUserId): array {
        ...
        if (!in_array($collab['status'], $allowedStatuses, true)) {
    ...
    public static function verifyEscrowPayment(array $params): array {
        ...
        if (!$isSimulatedOrder && !empty($razorpaySignature)) {
            ...
            if (!$signatureVerified) {
                $keyId = env('RAZORPAY_KEY_ID', '');
                // If using simulated signature or test sandbox key
                if (str_starts_with($keyId, 'rzp_test_') || str_starts_with($razorpaySignature, 'sig_') || str_starts_with($razorpaySignature, 'test_')) {
    ...
    /**
     * Create Subscription Order for Creator Tier Upgrade (₹1 Pro / Silver / Gold / Diamond)
     */
    public static function createSubscriptionOrder(string $creatorId, string $tier, float $price = 1.0): array {
        ...
        $receipt = "sub_" . substr($creatorId, 0, 16) . "_" . round(microtime(true) * 1000);

        $rzpOrder = self::callRazorpayOrderAPI($amountInPaise, $receipt, [
            'creator_id' => $creatorId,
            'tier' => $tier,
            'purpose' => "CreatorHub {$tier} Upgrade"
        ]);

        $orderId = $rzpOrder ? $rzpOrder['id'] : 'order_sub_sim_' . round(microtime(true) * 1000) . '_' . substr(bin2hex(random_bytes(3)), 0, 5);
        $isSimulated = empty($rzpOrder);

        $paymentId = 'pay_sub_' . round(microtime(true) * 1000) . '_' . substr(bin2hex(random_bytes(3)), 0, 5);
        Database::execute(
            "INSERT INTO payments (
                id, collaboration_id, brand_id, creator_id, amount, currency,
                payment_type, status, is_simulated, transaction_ref, razorpay_order_id
            ) VALUES (?, NULL, NULL, ?, ?, 'INR', 'Subscription Upgrade', 'PENDING', ?, ?, ?)",
            [
                $paymentId,
                $creatorId,
                $price,
        ...
        return [
            ...
            'prefill' => [
                ...
                'email' => $creator['username'] . '@creatorhub.com'
            ]
        ];
    }

    /**
     * Verify Subscription Upgrade Payment & Activate Creator Tier
     */
    public static function verifySubscriptionPayment(array $params): array {
        $creatorId = $params['creator_id'];
        $tier = strtolower(trim($params['tier']));
        $razorpayOrderId = $params['razorpay_order_id'];
        $razorpayPaymentId = $params['razorpay_payment_id'];
        $razorpaySignature = $params['razorpay_signature'] ?? '';

        $creator = Database::queryOne("SELECT * FROM creator_profiles WHERE id = ?", [$creatorId]);
        if (!$creator) throw new \Exception("Creator profile not found.");

        $payment = Database::queryOne(
            "SELECT * FROM payments WHERE razorpay_order_id = ? OR (creator_id = ? AND payment_type = 'Subscription Upgrade') ORDER BY created_at DESC LIMIT 1",
            [$razorpayOrderId, $creatorId]
        );

        $isSimulatedOrder = str_starts_with($razorpayOrderId, 'order_sub_sim_') || str_starts_with($razorpayOrderId, 'order_sim_');
        $signatureVerified = false;
        ...
        return Database::transaction(function() use ($payment, $creatorId, $tier, $razorpayOrderId, $razorpayPaymentId, $razorpaySignature, $signatureVerified, $isSimulatedOrder) {
            if ($payment) {
                Database::execute(
                    "UPDATE payments
            ...
            // Update creator profile subscription tier
            Database::execute(
            ...
            // Upsert creator_subscriptions record if table exists
            try {
                $subId = 'sub_' . round(microtime(true) * 1000) . '_' . substr(bin2hex(random_bytes(3)), 0, 5);
                Database::execute(
                    "INSERT INTO creator_subscriptions (
                        id, creator_id, tier, price, billing_cycle, status, payment_method, transaction_ref, started_at, created_at
                    ) VALUES (?, ?, ?, 1.0, 'monthly', 'active', 'Razorpay Gateway', ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)",
                    [
                        $subId,
                        $creatorId,
                        $tier,
                ...
                error_log("Note: creator_subscriptions insert log: " . $subErr->getMessage());
            }
            ...
            return [
                ...
                'verified' => true,
                'tier' => $tier,
                'plan_name' => ucfirst($tier) . ' VIP',
                'payment_id' => $razorpayPaymentId,
                'message' => "Congratulations! You have been successfully upgraded to " . ucfirst($tier) . " Tier."
            ];
        });
    }
