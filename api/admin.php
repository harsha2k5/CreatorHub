...
require_once dirname(__DIR__) . '/includes/controllers/AdminController.php';
...
function handleAdminRoute(array $segments, string $method, array $body) {
    ...
    if ($first === 'payments' && empty($second) && $method === 'GET') {
        AdminController::payments();
        return;
    }

    if ($first === 'applications' && empty($second) && $method === 'GET') {
