...
require_once dirname(__DIR__) . '/includes/controllers/InstagramController.php';
...
function handleInstagramRoute(string $action, string $method, array $body) {
    ...
    if ($action === 'config-status' && $method === 'GET') {
        InstagramController::configStatus();
        return;
    }

    if ($action === 'profile' && $method === 'GET') {
        InstagramController::profile();
        return;
    }

    if ($action === 'insights' && $method === 'GET') {
        InstagramController::insights();
        return;
    }
